...
$result = sql("SELECT DISTINCT * FROM patients RIGHT OUTER JOIN events ON patients.id = events.name_patient
WHERE events.prescription LIKE '%" . $word . "%'  ", $e0);
?>
...
<table class="table table-striped table-bordered">
	...
	<th class="text-center" style="color:#0066ff ; font-size: 15px">Prescription</th>
</thead>

<tbody>
<?php while ($order = db_fetch_assoc($result)) { ?>
		<tr>
			<td class="text-left"><a href="patients_view.php?SelectedID=<?= $order['name_patient'] ?>"><?php echo $order['last_name']; ?>  <?php echo " ,   " . $order['first_name']; ?></a></td>
	...
	<?php if ($order['image']) {
		?>
			...
			<td class="text-left"><?php echo $order['prescription']; ?></td>
